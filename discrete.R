...
####################generate dataset####################
generate.cr<-function(n.sample,beta0,beta1,beta2,beta3,p1,p2,p3,tt){
  #err<-runif(n.sample,-0.5,0.5)
  err<-rnorm(n.sample,0,0.25)
  c<-runif(n.sample,0,tt)
  I<-rbinom(n.sample,1,p1)
  z1<-rbinom(n.sample,1,p2)-0.5
  z2<-rbinom(n.sample,1,p3)-0.5
  log.t<-beta0*(1-I)+beta1*I+beta2*z1+beta3*z2+err
  t<-exp(log.t)
  delta<-sign(t<=c)
  y=pmin(t,c)
...
generate.ps<-function(n.sample,beta0,beta1,beta2,beta3,p1,p2,tt){
  #err<-runif(n.sample,-0.5,0.5)
  err<-rnorm(n.sample,0,0.25)
  c<-runif(n.sample,0,tt)
...
generate.spb<-function(n.sample,beta0,beta1,beta2,beta3,p1,p2,tt){
  #err<-runif(n.sample,-0.5,0.5)
  err<-rnorm(n.sample,0,0.25)
  c<-runif(n.sample,0,tt)
  z1<-rbinom(n.sample,1,p1)
...
power.cr(5000,500,10,0.5,0.5)
power.spb(5000,500,10,0.5,0.5)
power.ps(5000,500,10,0.5,0.5)

power.cr(5000,250,10,0.5,0.5)
power.spb(5000,250,10,0.5,0.5)
power.ps(5000,250,10,0.5,0.5)
