...
generate.cr<-function(n.sample,beta0,beta1,beta2,beta3,p1,p2,p3,t){
  ...
  sigma <- matrix(c(0.25,0.16,0.16,0.25),2,2)
  zz<-mvrnorm(n=n.sample, rep(0, 2), sigma)
  zz1<-zz[,1]
  #zz2<-rnorm(n.sample,0,1)
  zz2<-zz[,2]
  log.t<-beta0*(1-I)+beta1*I+beta2*zz1+beta3*zz2+err
  t<-exp(log.t)
  delta<-sign(t<=c)
  y=pmin(t,c)
  data<-data.frame(I,zz1,zz2,t,y,c,delta)
  return(data)
}
...
power.cr<-function(n.sim,n.sample,tt,beta0,beta1){
  ...
  for (k in 1:n.sim){ 
    ...
    model1 <- lm(log(y) ~ I+I1+zz1+zz2-1, weights=w,data=data1)
    model2 <- lm(log(y) ~ I+I1-1, weights=w,data=data1)
    model3 <- lm(log(y) ~ I+I1+zz1-1, weights=w,data=data1)
...
#########################Correlated CR n=500 censor=20%######################
power.cr(5000,500,10,.5,.5)
#########################Correlated PS n=500 censor=20%######################
power.ps(5000,500,10,.5,.5)
#########################Correlated SPB n=500 censor=20%######################
power.spb(5000,500,10,.5,.5)
#########################zCorrelated CR n=250 censor=20%######################
power.cr(5000,250,10,.5,.5)
#########################zCorrelated PS n=250 censor=20%######################
power.ps(5000,250,10,.5,.5)
#########################Correlated SPB n=250 censor=20%######################
